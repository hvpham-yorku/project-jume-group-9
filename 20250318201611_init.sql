...
CREATE OR REPLACE FUNCTION util.prevent_modifying_columns(NEW record, restricted_columns TEXT[])
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    -- Handle NULL inputs
    IF NEW IS NULL OR restricted_columns IS NULL THEN
        RETURN TRUE;
    END IF;
    ...
    -- Check if any restricted column exists in the NEW record
...
CREATE OR REPLACE FUNCTION policy.users_select(user_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF auth.uid() = user_id THEN -- auth.uid() is the current user's ID
        RETURN TRUE;
    ELSE
...
CREATE OR REPLACE FUNCTION policy.users_update(user_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR UPDATE 
TO authenticated                
USING (policy.users_update(id)) 
WITH CHECK (                    
    policy.users_update(id) 
...
CREATE OR REPLACE FUNCTION policy.users_delete(user_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION policy.orgs_select(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM org_members            -- is the at least 1 ?
        WHERE org_members.org_id = org_id    -- hey (current org_id) that you just inputted, are you the same in my table?
...
FOR SELECT 
TO authenticated 
USING (policy.orgs_select(id));



/* ⚒️🔒 ----------------------- RLS function:UPDATE ---------------------- 🧬 */
CREATE OR REPLACE FUNCTION policy.orgs_update(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR UPDATE 
TO authenticated 
USING (policy.orgs_update(id))
WITH CHECK (
    policy.orgs_update(id)
    AND util.prevent_modifying_columns(
        NEW, 
        ARRAY[
            'id',
...
/* ⚒️🔒 ---------------------- RLS function:DELETE ----------------------- ❌ */
CREATE OR REPLACE FUNCTION policy.orgs_delete(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
/* ⚒️🔒 ---------------------- RLS function:SELECT ----------------------- 👀 */
CREATE OR REPLACE FUNCTION policy.org_members_select(org_id UUID) -- this org_id is the from the record I want to view
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF EXISTS (
        ...
        WHERE org_members.org_id = org_id   -- ...so first If there exists a org_id in org_members AT ALL ...
...
FOR SELECT 
TO authenticated 
USING (policy.org_members_select(org_id));



/* ⚒️🔒 ----------------------- RLS function:INSERT ---------------------- 🔥 */
CREATE OR REPLACE FUNCTION policy.org_members_insert(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR INSERT 
TO authenticated
WITH CHECK (policy.org_members_insert(org_id));



/* ⚒️🔒 ----------------------- RLS function:UPDATE ---------------------- 🧬 */
CREATE OR REPLACE FUNCTION policy.org_members_update(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR UPDATE 
TO authenticated 
USING (policy.org_members_update(org_id));



/* ⚒️🔒 ---------------------- RLS function:DELETE ----------------------- ❌ */
CREATE OR REPLACE FUNCTION policy.org_members_delete(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR DELETE 
TO authenticated 
USING (policy.org_members_delete(org_id));
...
/* ⚒️🔒 ---------------------- RLS function:SELECT ----------------------- 👀 */
CREATE OR REPLACE FUNCTION policy.orders_select(orders_org_id UUID) -- this orders_org_id is from the Order record I want to view
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM org_members  -- the point is that org_id and user_id are together coupled with org_members...
        WHERE org_members.org_id = orders_org_id   -- ...so first If there exists a org_id in org_members AT ALL ...
...
FOR SELECT 
TO authenticated 
USING (policy.orders_select(org_id));




/* ⚒️🔒 ----------------------- RLS function:INSERT ---------------------- 🔥 */
CREATE OR REPLACE FUNCTION policy.orders_insert(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR INSERT 
TO authenticated
WITH CHECK (policy.orders_insert(org_id));




/* ⚒️🔒 ----------------------- RLS function:UPDATE ---------------------- 🧬 */
CREATE OR REPLACE FUNCTION policy.orders_update(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR UPDATE 
TO authenticated 
USING (policy.orders_update(org_id));




/* ⚒️🔒 ---------------------- RLS function:DELETE ----------------------- ❌ */
CREATE OR REPLACE FUNCTION policy.orders_delete(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR DELETE 
TO authenticated 
USING (policy.orders_delete(org_id));
...
/* ⚒️🔒 ---------------------- RLS function:SELECT ----------------------- 👀 */
CREATE OR REPLACE FUNCTION policy.customers_select(customers_org_id UUID) -- this customers_org_id is from the Customer record I want to view
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM org_members  -- the point is that org_id and user_id are together coupled with org_members...
        WHERE org_members.org_id = customers_org_id   -- ...so first If there exists a org_id in org_members AT ALL ...
        AND org_members.user_id = auth.uid() -- ...Since org_id + user_id comes together --> Then if my user id (auth.uid()) is the same as the org_id in the record --> we good (thats my org_id).
...
FOR SELECT 
TO authenticated 
USING (policy.customers_select(org_id));



/* ⚒️🔒 ----------------------- RLS function:INSERT ---------------------- 🔥 */
CREATE OR REPLACE FUNCTION policy.customers_insert(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR INSERT 
TO authenticated
WITH CHECK (policy.customers_insert(org_id));
...
/* ⚒️🔒 ----------------------- RLS function:UPDATE ---------------------- 🧬 */
CREATE OR REPLACE FUNCTION policy.customers_update(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR UPDATE 
TO authenticated 
USING (policy.customers_update(org_id));



/* ⚒️🔒 ---------------------- RLS function:DELETE ----------------------- ❌ */
CREATE OR REPLACE FUNCTION policy.customers_delete(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR DELETE 
TO authenticated 
USING (policy.customers_delete(org_id));
...
/* ⚒️🔒 ---------------------- RLS function:SELECT ----------------------- 👀 */
CREATE OR REPLACE FUNCTION policy.products_select(products_org_id UUID) -- this products_org_id is from the Product record I want to view
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM org_members  -- the point is that org_id and user_id are together coupled with org_members...
        WHERE org_members.org_id = products_org_id   -- ...so first If there exists a org_id in org_members AT ALL ...
        AND org_members.user_id = auth.uid() -- ...Since org_id + user_id comes together --> Then if my user id (auth.uid()) is the same as the org_id in the record --> we good (thats my org_id).
...
FOR SELECT 
TO authenticated 
USING (policy.products_select(org_id));




/* ⚒️🔒 ----------------------- RLS function:INSERT ---------------------- 🔥 */
CREATE OR REPLACE FUNCTION policy.products_insert(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR INSERT 
TO authenticated
WITH CHECK (policy.products_insert(org_id));
...
/* ⚒️🔒 ---------------------- RLS function:DELETE ----------------------- ❌ */
CREATE OR REPLACE FUNCTION policy.products_delete(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR DELETE 
TO authenticated 
USING (policy.products_delete(org_id));
...
/* ⚒️🔒 ---------------------- RLS function:SELECT ----------------------- 👀 */
CREATE OR REPLACE FUNCTION policy.product_restock_notifications_select(product_restock_notifications_org_id UUID) -- this product_restock_notifications_org_id is from the Product record I want to view
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM org_members  -- the point is that org_id and user_id are together coupled with org_members...
        WHERE org_members.org_id = product_restock_notifications_org_id   -- ...so first If there exists a org_id in org_members AT ALL ...
        AND org_members.user_id = auth.uid() -- ...Since org_id + user_id comes together --> Then if my user id (auth.uid()) is the same as the org_id in the record --> we good (thats my org_id).
...
FOR SELECT 
TO authenticated 
USING (policy.product_restock_notifications_select(org_id));




/* ⚒️🔒 ---------------------- RLS function:DELETE ----------------------- ❌ */
CREATE OR REPLACE FUNCTION policy.product_restock_notifications_delete(org_id UUID)
RETURNS BOOLEAN 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
FOR DELETE 
TO authenticated 
USING (policy.product_restock_notifications_delete(org_id));
