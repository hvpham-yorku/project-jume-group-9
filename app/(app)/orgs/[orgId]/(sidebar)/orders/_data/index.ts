...
export async function getOrders({ supabase, params }: { supabase: Client; params: GetOrdersParams }) {
  return await unstable_cache(
    async () => {
      ...
      // Pagination, based on a range because of Supabase (perPage = page range)
...
// Checks the available range and gives min and max range of total price
export async function getOrdersTotalRange({ supabase, orgId }: { supabase: Client; orgId: string }) {
  return await unstable_cache(
    async () => {
      // Sorted by ascending, first item = min
      const { data: minData } = await supabase
        .from("orders")
        .select("total")
        .eq("org_id", orgId)
        .order("total", { ascending: true })
        .limit(1) // Gets the first one item
        .single()
        .throwOnError();

      // Sorted by descending, first item = max
...
// Same range situation as price (above)
