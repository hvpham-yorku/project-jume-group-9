...
/* ⚡️ ---- trigger: monitor inventory for restock threshold on UPDATE---- ⚡️ */
...
/* ⚡️ --- trigger: monitor inventory for restock threshold on INSERT --- ⚡️ */
...
/* ⚒️⚡️ ----- function: update product inventory when order fulfilled ----- ⚒️⚡️ */
...
/* ⚡️ ---- trigger: update inventory when order fulfillment_status changes to fulfilled ---- ⚡️ */
...
BEGIN
    IF EXISTS (
        ...
        RETURN TRUE;
    ELSE
        RAISE EXCEPTION 'Error: You must be a member of this org to view its activity logs';
    END IF;
END;
$$;

/* 🔒 ------------------------- RLS policy:SELECT ------------------------ 👀 */
CREATE POLICY "Members can view activity logs in their orgs" 
ON activity_logs 
FOR SELECT 
TO authenticated 
